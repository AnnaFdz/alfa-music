...
function DeletePlaylist() {
    ...
    const handleClick = (playlistId, playlistName) => {
        navigate("/confirmDelete", { state: { id: playlistId, name: playlistName } });
    };

    return (
        <div className="container modify-playlist-container">
            <h1 className="title modify-playlist-title">Eliminar Playlists</h1>
            {isLoading ? (
                    ...
                    {paginatedPlaylists.length > 0 ? (
                        paginatedPlaylists.map((playlist) => (
                            <div className="column is-one-quarter" key={playlist.id}>
                                <div className="card modify-playlist-card" onClick={() => handleClick(playlist.id, playlist.name)}>
